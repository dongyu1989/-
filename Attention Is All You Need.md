...
如表1所示，一个自我关注层将所有位置与恒定数量的顺序执行操作连接起来，而一个循环层则需要O(N)顺序操作。在计算复杂度方面，当序列长度n小于表示维数d时，自注意层比循环层更快，这是机器翻译中最先进的模型（如word-piece[38]和byte-pair[31] representations）使用的句子表示最常见的情况。为了提高涉及非常长序列的任务的计算性能，可以将自我关注限制为仅考虑以各自输出位置为中心的输入序列中大小为r rr的邻域。这将把最大路径长度增加到O(n/r)。我们计划在今后的工作中进一步研究这种方法。
